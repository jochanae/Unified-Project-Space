...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.beta_serial_numbers
    WHERE user_id = p_test_user_id AND serial_number = 0
  ) THEN
  ...
  SET notified_at = NULL, snapshot_saved_at = NULL
  WHERE user_id = p_test_user_id;
