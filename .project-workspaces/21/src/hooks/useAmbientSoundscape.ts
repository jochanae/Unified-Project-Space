// Ambient soundscapes per Sound Suite deck — procedural Web Audio
import { logger } from '@/utils/logger';
import { useEffect, useRef, useCallback, useState } from 'react';

export type SoundDeckId = 'brown' | 'rain' | 'synth' | 'lofi' | 'fire' | 'none';

// Legacy compat for Circle features
import type { CircleType } from '@/components/circle/SpatialRoom';

export interface SoundscapeDeck {
  id: string;
  label: string;
  emoji: string;
...
export const SOUNDSCAPE_DECKS: SoundscapeDeck[] = [
  { id: 'lofi', label: 'Lo-Fi', emoji: '🎧', circleTypes: ['social'] },
  { id: 'rain', label: 'Rain', emoji: '🌧️', circleTypes: ['personal'] },
  { id: 'brown', label: 'Brown', emoji: '🌿', circleTypes: ['circle', 'service'] },
  { id: 'fire', label: 'Fireplace', emoji: '🔥', circleTypes: ['personal'] },
  { id: 'none', label: 'Off', emoji: '🔇', circleTypes: [] },
];

export function getDefaultDeck(circleType: CircleType): SoundDeckId {
  if (circleType === 'kids') return 'none';
  const found = SOUNDSCAPE_DECKS.find(d => d.circleTypes.includes(circleType));
  return (found?.id as SoundDeckId) || 'none';
}
...
function createSynthCosmos(ctx: AudioContext, master: GainNode) {
  ...
  // Shimmering synth pad — slow detuned sines
  const freqs = [174.6, 220, 261.6, 349.2]; // F3 A3 C4 F4
  freqs.forEach((f, idx) => {
...
function createFireCrackling(ctx: AudioContext, master: GainNode) {
  ...
  // Deep rumble — low frequency fire body
  const bufferSize = ctx.sampleRate * 4;
...
export function useAmbientSoundscape(circleTypeOrEnabled?: CircleType | boolean, enabled: boolean = true) {
  // Support both signatures: useAmbientSoundscape(enabled) and useAmbientSoundscape(circleType, enabled)
  const isEnabled = typeof circleTypeOrEnabled === 'boolean' ? circleTypeOrEnabled : enabled;
  const ctxRef = useRef<AudioContext | null>(null);
  const masterRef = useRef<GainNode | null>(null);
  const nodesRef = useRef<AudioNode[]>([]);
  const [activeDeck, setActiveDeck] = useState<SoundDeckId>('none');
  ...
  const start = useCallback((deckId: SoundDeckId) => {
    cleanup();
    ...
    try {
      ...
      // 2-second fade in
      master.gain.linearRampToValueAtTime((volume / 100) * 0.6, ctx.currentTime + 2);
    ...
    } catch (e) { logger.warn("[AmbientSoundscape] Failed:", e); }
  }, [cleanup, volume]);
  ...
  const switchDeck = useCallback((deckId: string) => {
    const id = deckId as SoundDeckId;
    setActiveDeck(id);
    start(id);
  }, [start]);
  ...
  return { activeDeck, switchDeck, volume, setVolume, cleanup };
}
