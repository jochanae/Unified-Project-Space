...
interface FooterShortcutEditorProps {
  ...
  leftIds: string[];
  rightIds: string[];
  onSave: (left: string[], right: string[]) => Promise<void>;
}
...
export function FooterShortcutEditor({ open, onOpenChange, leftIds, rightIds, onSave }: FooterShortcutEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave(editLeft, editRight);
      onOpenChange(false);
      toast.success('Footer shortcuts updated');
    } catch (error) {
      console.error('Failed to save footer shortcuts:', error);
      toast.error('Could not save footer shortcuts');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogContent className="max-w-sm">
        ...
        <Button onClick={handleSave} className="w-full mt-3" disabled={isSaving}>
