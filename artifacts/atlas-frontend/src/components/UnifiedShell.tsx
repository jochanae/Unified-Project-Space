...
function ShellConversationTitle({ title }: { title: string | null }) {
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, maxWidth: "min(260px, 100%)", minWidth: 0 }}>
      ...
      {/* New conversation button — fires axiom:home-reset which calls
          handleNewConversation() in home.tsx, clearing localStorage and
          returning the user to the fresh ambient state. Without this there
          is no discoverable escape from a restored conversation on mobile. */}
      <button
        aria-label="New conversation"
        title="New conversation"
        onClick={() => window.dispatchEvent(new Event("axiom:home-reset"))}
        style={{
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        ...
        onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.opacity = "1"; (e.currentTarget as HTMLButtonElement).style.color = "var(--atlas-fg)"; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.opacity = "0.7"; (e.currentTarget as HTMLButtonElement).style.color = "var(--atlas-muted)"; }}
      >
        {/* SquarePen icon (lucide) — inlined SVG so no import needed */}
